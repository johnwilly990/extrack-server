...
const sumEntry = async (userId) => {
  ...
  const sumInvetmentsArr = await db("investments_entries")
    .where("user_id", userId)
    .sum("amount");

  // Retrieves sum value
  const sumOfInvetments = Object.values(sumInvetmentsArr[0])[0];

  // Updates Users table with summed value
  await db("users")
    .where({ id: userId })
    .update({ investment_amount: sumOfInvetments });
};
...
exports.addEntry = async (req, res) => {
  try {
    ...
    await db("investments_entries").insert(newEntry);
...
exports.updateEntry = async (req, res) => {
  try {
    ...
    // Inserts new entry to db
    await db("investments_entries").where({ id: id }).update(entry);
...
exports.deleteEntry = async (req, res) => {
  try {
    ...
    const databaseId = await db("investments_entries").where({ id: id });
    ...
    await db("investments_entries").del().where({ id: id });
...
exports.getAllEntries = async (req, res) => {
  ...
  const users = await db("investments_entries")
    .where({
      user_id: req.userId,
    })
