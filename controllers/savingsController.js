...
const sumEntry = async (userId) => {
  ...
  const sumSavingsArr = await db("savings_entries")
    .where("user_id", userId)
    .sum("amount");

  // Retrieves sum value
  const sumOfSavings = Object.values(sumSavingsArr[0])[0];

  // Updates Users table with summed value
  await db("users")
    .where({ id: userId })
    .update({ saving_amount: sumOfSavings });
};
...
exports.addEntry = async (req, res) => {
  try {
    ...
    await db("savings_entries").insert(newEntry);
...
exports.updateEntry = async (req, res) => {
  try {
    ...
    // Inserts new entry to db
    await db("savings_entries").where({ id: id }).update(entry);
...
exports.deleteEntry = async (req, res) => {
  try {
    ...
    const databaseId = await db("savings_entries").where({ id: id });
    ...
    await db("savings_entries").del().where({ id: id });
...
exports.getAllEntries = async (req, res) => {
  ...
  const users = await db("savings_entries")
    .where({
      user_id: req.userId,
    })
